...
int main(int argc, char **argv)
{
  ...
  else
  {
    ...
    if (argument == "kalman_filter")
      filter_name = KALMAN;
    else if (argument == "IE_kalman_filter")
      filter_name = IE_KALMAN;
    else if (argument == "bootstrap_filter")
      filter_name = BOOTSTRAP;
    else if (argument == "EK_particle_filter")
      filter_name = EK_PARTICLE;
    else if (argument == "ASIR_filter")
      filter_name = ASIR;
    else
    {
  ...
  prior_cont.SampleFrom(prior_samples, NUM_SAMPLES, CHOLESKY, NULL);//SampleMthd::CHOLESKY
  prior_discr.ListOfSamplesSet(prior_samples);

  cout << "Prior initialised"
       << "" << endl;
  cout << "Prior Mean = " << endl
       << prior_cont.ExpectedValueGet() << endl
       << "Covariance = " << endl
       << prior_cont.CovarianceGet() << endl;
  ...
  cout << " Posterior Mean = " << endl
       << posterior->ExpectedValueGet() << endl
       << " Covariance = " << endl
       << posterior->CovarianceGet() << "" << endl;
  cout << "=============================================" << endl;
